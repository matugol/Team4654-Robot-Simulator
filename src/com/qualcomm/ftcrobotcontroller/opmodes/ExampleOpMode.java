...
public class ExampleOpMode extends BaseOpMode {
	float driveModeButton, driveControlStickX, driveControlStickY, tankModeButton, tankLeftControlStickY, tankRightControlStickY, splitModeButton, splitControlStickX, splitControlStickY;

	private State state = State.DRIVE;
	private final boolean climberButtonPressed = false, climberHolderDown = false, lCPressed = false, rCPressed = false, leftClimberDown = false, rightClimberDown = false, lHPressed = false, rHPressed = false, lHUp = false, rHUp = false;
	...
	public void loop() {
		...
		if (driveModeButton >= 0.75) state = State.DRIVE;
		if (tankModeButton >= 0.75) state = State.TANK;
		if (splitModeButton >= 0.75) state = State.SPLIT;
		...
		switch (state) {
			...
			case SPLIT:
				// SPLIT mode
				// Very similar to drive mode, except that two seperate control sticks control the forward/backward axis and the
// left/right axis..
